...
from ..resources import console, resources_dir_paths, default_transient_progress
import requests
import tarfile
import shutil
...
def download_archive(archive_link, archive_path, archive_name=""):
    ...
    with open(archive_path, "wb") as archive:
        with default_transient_progress() as progress:
            task_id = progress.add_task("Downloading {}archive...".format(archive_name), start=False)

            response = requests.get(archive_link, stream=True, headers={"Accept-Encoding": ""})
...
def move(source, target, source_name=""):
